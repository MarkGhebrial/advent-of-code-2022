...
fn find_dir(dir: &Node, min_size: usize) -> usize {
    ...
    match dir {
        ...
        Node::Directory { name: _, children } => {
            ...
            if size < best_size && size >= min_size {
                best_size = size;
            }

            for child in children.borrow().iter() {
                let size = find_dir(&child, min_size);
                if size < best_size && size >= min_size {
                    best_size = size;
                }
            }
...
fn main() {
    ...
    let space_available = 70000000 - root.get_size();

    let size = find_dir(&root, 30000000 - space_available);
    println!("{}", size);
}
